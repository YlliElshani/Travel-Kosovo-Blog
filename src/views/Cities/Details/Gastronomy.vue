...
import getGastronomyList from "../../../utility/City/getGastronomyList";
...
export default {
	...
	computed: {
		...mapGetters({
			citiesGastronomy: "citiesGastronomy",
		}),
		gastronomies() {
	...
	methods: {
		...
		async fetchCitiesGastronomy() {
			const result = await getGastronomyList(this.$route.params.id);
			// console.log(result);
			this.$store.dispatch("fetchCitiesGastronomy", result);
		},
	},
};
